...
type BootstrapConfig struct {
	...
	// to verify the .snap at JujuDbSnap
...
func (cfg *InstanceConfig) SetSnapSource(snapPath string, snapAssertionsPath string) error {
	...
	if err != nil {
		return errors.Annotatef(err, "unable to find snap at %s", snapPath)
	}

	_, err = os.Stat(snapAssertionsPath)
	if err != nil {
		return errors.Annotatef(err, "unable to find .assert at %s", snapAssertionsPath)
	}
